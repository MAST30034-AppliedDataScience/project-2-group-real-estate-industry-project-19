        ...
        for page in pages:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"Received status code {response.status_code} for {url}. Exiting page loop.")
                    break  # Exit the page loop and move to the next suburb
...
def scrape_oldlistings_data(urls):
    ...
    for property_url in pbar:
        ...
        # Add a random delay between each URL request to avoid hitting the server too frequently
        delay = random.uniform(1, 5)  # Random delay between 1 to 3 seconds
...
def process_suburbs_in_batches(suburbs, pages, batch_size=5, delay=30):
    """
    Processes suburbs in batches, scrapes data, and saves it to CSV.

    Parameters:
    suburbs (list): List of suburbs to scrape.
    pages (range): Range of pages to scrape for each suburb.
    batch_size (int): Number of suburbs to process in each batch.
    delay (int): Delay in seconds between processing batches.
    """
    total_batches = len(suburbs) // batch_size + (1 if len(suburbs) % batch_size else 0)
    print(f"Total batches to process: {total_batches}")

    batch_number = 3

    for i in range(0, len(suburbs), batch_size):
        # Get the current batch of suburbs
        batch_suburbs = suburbs[i:i + batch_size]
        print(f"Processing batch {batch_number} of {total_batches}: {batch_suburbs}")

        # Generate URLs for the current batch
        urls = test_urls(pages, batch_suburbs)

        # Scrape property data
        property_data = scrape_oldlistings_data(urls)

        # Save the scraped data to a CSV file with a sequential batch number
        output_filename = f'data/landing/rental_history_scrape_batch_{batch_number}.csv'
        save_data_to_csv(property_data, output_filename)
        print(f"Data saved to {output_filename}")

        # Increment the batch number for the next iteration
        batch_number += 1

        # If this is not the last batch, wait before moving on to the next batch
        if i + batch_size < len(suburbs):
...
process_suburbs_in_batches(SUBURBS, N_PAGES, batch_size=10, delay=30)